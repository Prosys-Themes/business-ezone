...
// register Business_Ezone_Featured_Post widget
function business_ezone_register_featured_post_widget() {
    register_widget( 'Business_Ezone_Featured_Post' );
}
add_action( 'widgets_init', 'business_ezone_register_featured_post_widget' );
 
 /**
 * Adds Business_Ezone_Featured_Post widget.
 */
class Business_Ezone_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'business_ezone_featured_post', // Base ID
			__( 'PROSYS: Featured Post', 'business-ezone' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'business-ezone' ), ) // Args
		);
	}
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'business-ezone-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
		$postlist[0] = array(
    		'value' => 0,
    		'label' => __('--Choose--', 'business-ezone'),
    	);
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'business-ezone' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'business-ezone' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'business-ezone' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'business-ezone' );
...
} // class Business_Ezone_Featured_Post